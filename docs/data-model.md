...
Job은 `{ id, accountId, provider, handle, slotNumber, videoId, status, progress, attempt, maxAttempts, nextRetryAt, lastError, externalId, externalUrl, mode, analytics, logs }`를 가진다. TikTok live 게시가 확인되면 `externalUrl`에 실제 `/video/{id}` 주소가 저장된다.
...
- Settings: `launchAtStartup`, `startMinimized`, `autoUpdate`, `providerMode`(`sandbox`/`live`), `maxAttempts`
