...
class NaverClipProviderAdapter extends ProviderAdapter {
  ...
  async #createClient() {
    if (this.clientFactory) return this.clientFactory({ userDataDir: this.userDataDir, headless: this.headless, timeoutMs: this.timeoutMs, logger: this.logger });
    const { NaverClipClient } = await import('naver-clip-integration');
    return new NaverClipClient({ userDataDir: this.userDataDir, headless: this.headless, timeoutMs: this.timeoutMs, logger: this.logger });
  }
}
...
class InstagramProviderAdapter extends ProviderAdapter {
  ...
  async #createClient() {
    ...
    return new InstagramClient({ userDataDir: this.userDataDir, headless: this.headless, timeoutMs: this.timeoutMs, logger: this.logger });
  }
}
...
class FacebookProviderAdapter extends ProviderAdapter {
  ...
  async publish({ video, campaign, job }) {
    ...
    return enqueueFacebookPublish(async () => {
      ...
      try {
        ...
        const results = await client.uploadVideos({
          finalize: true,
          videos: [{ filePath: video.filePath, pageHandle: job?.handle, ...facebookMetadata({ video, campaign, job }) }]
        });
        const published = results?.[0];
        if (!published?.externalId || !published?.url) {
  ...
  async #createClient() {
    ...
    return new FacebookClient({ userDataDir: this.userDataDir, headless: this.headless, timeoutMs: this.timeoutMs, logger: this.logger });
  }
}
