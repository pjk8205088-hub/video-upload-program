...
const { normalizeNaverCategory } = require('./naver-category');
...
function naverClipMetadata({ video, campaign, job }) {
  ...
  const category = [clip.primaryCategory, clip.secondaryCategory].map(normalizeNaverCategory).filter(Boolean);
  const result = {
    caption: [description, hashtags.join(' ')].filter(Boolean).join('\n\n').slice(0, 300),
    category: category.length ? category : ['엔터', '엔터'],
