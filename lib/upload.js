...
async function executeUploadJob({ job, video, campaign, adapterFactory = getProviderAdapter, maxAttempts = 3, retryBaseMs = 1000, now = () => new Date(), terminalErrorPredicate = () => false }) {
  ...
  try {
    ...
    if (terminalErrorPredicate(error)) {
      job.status = 'failed';
      job.nextRetryAt = null;
      addJobLog(job, `사용자 확인이 필요한 업로드 오류: ${error.message}`, 'error', () => nowDate().toISOString());
    } else if (job.attempt < (job.maxAttempts || maxAttempts)) {
...
module.exports = { validSlot, buildUploadRoutes, addJobLog, selectReadyJobs, updateUploadStatus, executeUploadJob };
